...
const NEW_ONCHOPRISTIS_MEDIA = [
  {
    url: 'https://bbsmxcoywionsvmfznah.supabase.co/storage/v1/object/public/species-media/1649-Onchopristis.png',
    type: 'art',
    credit: 'Gasmasque (CC BY-SA 4.0)',
    sourceUrl: 'https://commons.wikimedia.org/wiki/File:Onchopristis%20numida.png'
  }
];

async function main() {
  console.log('═══════════════════════════════════════════════════════════════════════════');
  console.log('🏛️  PREHISTORICA AUDITED REVISION: Onchopristis numidus (#1649) Media');
  console.log('═══════════════════════════════════════════════════════════════════════════\n');
  ...
  // Step 2: Locate Onchopristis
  const onchopristis = snapshotMap.get(1649);
  if (!onchopristis) {
    throw new Error('Onchopristis (#1649) not found in database!');
  }
  console.log(`Target: #${onchopristis.id} (${onchopristis.name})`);
  console.log('Current media:', onchopristis.media, '\n');

  // Step 3: Update Onchopristis media in database
  console.log('Step 3: Updating Onchopristis media in database...');
  const newMediaStr = JSON.stringify(NEW_ONCHOPRISTIS_MEDIA);
  await prisma.species.update({
    where: { id: 1649 },
    data: { media: newMediaStr }
  });
  console.log('✅ Onchopristis media updated: removed Knüppe group artwork, kept Gasmasque restoration as primary art.\n');
  ...
  for (const after of afterSpecies) {
    ...
    if (after.id === 1649) {
      for (const field of PROTECTED_FIELDS) {
        ...
        if (bNorm !== aNorm) {
          targetViolations.push(`Onchopristis field '${field}' was unintentionally modified!`);
        }
      }
      const parsedMedia = JSON.parse(after.media || '[]');
      if (parsedMedia.length !== 1 || parsedMedia[0].url !== NEW_ONCHOPRISTIS_MEDIA[0].url || parsedMedia[0].type !== 'art') {
        targetViolations.push(`Onchopristis media did not match expected value: ${after.media}`);
      }
    } else {
      for (const field of PROTECTED_FIELDS) {
        const bNorm = canonicalNormalize((before as any)[field]);
  ...
  console.log(`  - Exactly 1 targeted record updated: Onchopristis numidus (#1649) media`);
  console.log('  - All 23 non-media protected fields on Onchopristis remain 100% untouched');
  ...
  for (const filePath of jsonFilesToSync) {
    ...
    if (Array.isArray(data)) {
      for (const item of data) {
        if (item.id === 1649 || item.name === 'Onchopristis numidus') {
