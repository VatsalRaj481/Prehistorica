...
export default function RunwayMatchupModal({ isOpen, onClose, speciesList }: RunwayMatchupModalProps) {
  ...
  // Stop Lenis and background scroll while modal is active so wheel events belong strictly to modal
  useEffect(() => {
    if (!isOpen) return;
    lenis?.stop();
    ...
    <AnimatePresence>
      {isOpen && (
        <div
          ...
          className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/80 backdrop-blur-md overscroll-contain"
        >
          <motion.div
            ...
            data-lenis-prevent
            onWheel={(e) => {
              // Ensure mouse wheel scrolling works anywhere over the modal dialog
              if (contentRef.current && e.target !== contentRef.current && !contentRef.current.contains(e.target as Node)) {
                contentRef.current.scrollTop += e.deltaY;
              }
            }}
            className="relative w-full max-w-4xl max-h-[86vh] sm:max-h-[85vh] flex flex-col bg-slate-900 border border-white/[0.12] rounded-2xl shadow-2xl overflow-hidden font-sans text-slate-100"
          >
            {/* Header */}
