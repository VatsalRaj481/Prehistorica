...
export default function CompareModal({ initialSpecies, isOpen, onClose }: CompareModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 overflow-y-auto">
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-4xl p-6 shadow-2xl space-y-6 relative max-h-[90vh] overflow-y-auto">
        {/* Header */}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 bg-slate-950/60 p-4 rounded-xl border border-slate-855">
          ...
          <select
            ...
            disabled={loading}
            className="p-2 bg-slate-900 border border-slate-800 rounded-lg text-sm text-slate-200 focus:outline-none focus:border-blue-500 flex-grow max-w-xs disabled:opacity-50"
          >
            ...
            {availableList
              ...
              .map(s => (
                <option key={s.id} value={s.id}>
                  {s.name} ({s.clade} &bull; {s.timePeriod})
                </option>
              ))}
          </select>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 sm:gap-8 pt-2">
          {/* Species 1 Column */}
          {species1 && (
            <div className="space-y-4 bg-slate-950/40 p-4 rounded-xl border border-slate-855">
              <div className="relative h-40 bg-slate-950 rounded-lg overflow-hidden border border-slate-800 flex items-center justify-center p-2">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-100">{species1.name}</h3>
              ...
              <div className="space-y-2 text-xs text-slate-300">
                ...
                <div className="flex justify-between border-b border-slate-850 pb-1">
                  ...
                  <span className="font-bold text-indigo-400">{species1.clade}</span>
                </div>
                <div className="flex justify-between border-b border-slate-850 pb-1">
          ...
          {species2 ? (
            <div className="space-y-4 bg-slate-950/40 p-4 rounded-xl border border-slate-855">
              ...
              <div>
                <h3 className="text-lg font-bold text-slate-100">{species2.name}</h3>
              ...
              <div className="space-y-2 text-xs text-slate-300">
                ...
                <div className="flex justify-between border-b border-slate-850 pb-1">
                  <span className="text-slate-500 font-semibold flex items-center gap-1"><Dna className="h-3 w-3" /> Clade</span>
                  <span className="font-bold text-indigo-400">{species2.clade}</span>
                </div>
                <div className="flex justify-between border-b border-slate-850 pb-1">
                  <span className="text-slate-500 font-semibold flex items-center gap-1"><MapPin className="h-3 w-3" /> Formation</span>
            ...
            <div className="bg-slate-950/40 p-8 rounded-xl border border-slate-850 flex items-center justify-center text-slate-500 text-xs">
              Select a species above to compare
            </div>
          )}
        </div>
      </div>
